  ...
  async.forever(
    function (next) {
      ...
      if (program.startTime) {
        var momentStartTime = moment(program.startTime);
        startTime = momentStartTime.format('x');
        startFromHead = false;
      }
      ...
      var opts = {
        ...
        //not sure what this means, but is definately required for sending a start time, doesnt seem to break if not
        startFromHead: true
      };
      ...
      logs.getLogEvents(opts, function (err, res) {
        if (err) {
          return next(err);
        }
        else {
